...
def _build_experiment_task_desc(topic: str, detail: str) -> str:
    desc = (
        ...
        # ── STEP 0 (mandatory) ──────────────────────────────────────────
        # Decoupled from execution constraints on purpose. The agent
        # decides "what does the topic naturally cover?" BEFORE seeing
        # CPU/RAM/time limits, so the cheap signal ("make it small")
        # cannot contaminate the scope decision and silently shrink the
        # paper to a single dataset / single env / two algorithms.
        "STEP 0 — SCOPE LOCK (mandatory, do this BEFORE reading the "
        "EXECUTION ENVIRONMENT block below).\n"
        "Your very first tool call MUST be `submit_research_plan`. Until "
        "you call it, every other tool returns a hard error.\n"
        "When deciding the plan, derive scope FROM THE TOPIC ALONE — ignore "
        "execution constraints. The plan separates 'natural scope' from "
        "'what I will actually run', so CPU limits live in the split, not "
        "in the topic.\n"
        "  • natural_scope.methods: ≥3 distinct methods/algorithms when the "
        "topic uses plural framing ('compare different X', 'multiple Y "
        "methods', '不同的…').\n"
        "  • natural_scope.axes_of_comparison: dimensions you'll contrast "
        "(accuracy / sample efficiency / runtime / robustness / …).\n"
        "  • feasibility_split.will_execute: methods you can run end-to-end.\n"
        "  • feasibility_split.literature_only: methods covered only via "
        "literature synthesis (CPU / GPU / time / infra reasons). USE THIS "
        "BUCKET — do not silently drop methods just because they don't fit.\n"
        "  • title_hypothesis: the paper title. It MUST mirror the topic-"
        "level question, not the smallest dataset/env you'll happen to use. "
        "If the topic is 'compare different RL algorithms' and you can only "
        "run on CartPole, the title is still 'Comparing RL algorithms "
        "(evaluated on CartPole as a unified benchmark)' — NOT 'RL on "
        "CartPole'.\n"
        "  • rationale: 2-4 sentences justifying the methods, the split, "
        "and the title.\n\n"
        # ── Execution workflow (steps 1-5 happen AFTER scope is locked) ──
        "After STEP 0, your workflow:\n"
        "1. Literature Review: search for key methods, baselines, datasets, "
        "and evaluation metrics. Cover BOTH the will_execute and "
        "literature_only buckets — the literature_only methods need to "
        "appear in Related Work / Methodology even though they won't run.\n"
        "2. Plan execution: decide how to evaluate the will_execute methods "
        "— pick a scale that fits the EXECUTION ENVIRONMENT block below.\n"
        "3. Implement: write and run code incrementally in Python.\n"
        "4. Analyze: save tables as CSV and charts as PNG/SVG, then verify "
        "the findings.\n"
        "5. Report: call generate_report with verified references plus the "
        "real figure/table files. A topic-fidelity critic will compare your "
        "report against the locked scope plan; if the title or coverage has "
        "drifted, your generate_report call is rejected once and you must "
        "rewrite. After a second rejection the report is accepted with an "
        "auto-injected Limitations paragraph.\n"
    )
    if detail.strip():
...
def _build_survey_task_desc(topic: str, detail: str) -> str:
    ...
    desc = (
        ...
        # Scope lock applies to survey mode too. ``will_execute`` will be
        # empty (survey mode forbids code execution); ``literature_only``
        # carries the methods you'll cover in the synthesis. The title
        # hypothesis still has to mirror the topic — same anti-drift
        # contract as experiment mode.
        "STEP 0 — SCOPE LOCK (mandatory). Your very first tool call MUST be "
        "`submit_research_plan`. Until you call it, every other tool returns "
        "a hard error.\n"
        "  • natural_scope.methods: ≥3 distinct methods when the topic uses "
        "plural framing.\n"
        "  • natural_scope.axes_of_comparison: how the survey will contrast "
        "the methods (taxonomy, performance claims, assumptions, …).\n"
        "  • feasibility_split.will_execute: leave EMPTY — survey mode does "
        "not run code.\n"
        "  • feasibility_split.literature_only: every method you plan to "
        "cover.\n"
        "  • title_hypothesis: mirrors the topic, not the narrowest sub-area.\n"
